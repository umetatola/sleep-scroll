...
async function broadcastPhase() {
  const data = await chrome.storage.local.get(['sleepMode', 'windDownStyle', 'mode', 'targetSites']);
  ...
  if (timeRemaining === 0) {
    ...
    await chrome.storage.local.set({ sleepMode: { active: false } });
    chrome.alarms.clear('sleep-tick');
    broadcastClear();
    return;
  }
  ...
  if (phase !== sleepMode.phase) {
    await chrome.storage.local.set({ sleepMode: { ...sleepMode, phase } });
  }
...
async function recordSession(startTime, durationMinutes, completed) {
  ...
  await chrome.storage.local.set({ sessions: sessions.slice(-60) }); // keep last 60
}

async function triggerScheduledSleep() {
  const { schedule, sleepMode } = await chrome.storage.local.get(['schedule', 'sleepMode']);
  ...
  chrome.alarms.create('sleep-tick', { periodInMinutes: 0.5 });
  broadcastPhase();
}
...
function broadcastClear() {
  chrome.tabs.query({}, (tabs) => {
    for (const tab of tabs) {
      if (!isSocialMedia(tab.url)) continue;
      chrome.tabs.sendMessage(tab.id, { type: 'SLEEP_MODE_CLEAR' }).catch(() => {});
    }
  });
}
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (message.type === 'SET_SCHEDULE') {
    ...
    chrome.storage.local.set({ schedule: { enabled, time, duration } }).then(() => {
      chrome.alarms.clear('schedule-alarm');
      if (enabled) {
        chrome.alarms.create('schedule-alarm', {
  ...
  if (message.type === 'GET_SLEEP_MODE') {
    chrome.storage.local.get(['sleepMode', 'windDownStyle', 'schedule', 'mode', 'targetSites'], (data) => {
      sendResponse({
...
chrome.runtime.onInstalled.addListener(() => {
  chrome.storage.local.set({ sleepMode: { active: false } });
});
