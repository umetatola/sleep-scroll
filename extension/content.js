...
// ── Init: sync state when page loads mid-session ─────────────────────────────

chrome.runtime.sendMessage({ type: 'GET_SLEEP_MODE' }, (sm) => {
  if (chrome.runtime.lastError || !sm?.active) return;

  windDownStyle = sm.windDownStyle || 'breathing';
  mode = sm.mode || 'normal';
  targetSites = sm.targetSites || null;
  ...
  if (phase === 1) renderPhase1(timeRemaining);
  else if (phase === 2) renderPhase2(timeRemaining);
  else if (phase === 3) renderPhase3(timeRemaining, windDownStyle);
});
