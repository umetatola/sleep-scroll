...
document.querySelectorAll('.style-btn').forEach((btn) => {
  btn.addEventListener('click', () => {
    ...
    browser.storage.local.set({ windDownStyle: style }).then(() => {
...
document.getElementById('btn-settings').addEventListener('click', () => {
  // Open options page as a new tab (works on both macOS and iOS Safari)
...
document.getElementById('btn-start').addEventListener('click', () => {
  browser.runtime.sendMessage({ type: 'START_SLEEP_MODE', duration: selectedDuration }).then(() => {
    showActive();
    startTick();
  });
});
...
document.getElementById('btn-stop').addEventListener('click', () => {
  browser.runtime.sendMessage({ type: 'STOP_SLEEP_MODE' }).then(() => {
    showInactive();
    stopTick();
  });
});
...
browser.storage.local.get(['sleepMode', 'windDownStyle', 'schedule']).then(({ sleepMode, windDownStyle, schedule }) => {
  if (!windDownStyle) {
    showOnboarding();
    return;
  }
